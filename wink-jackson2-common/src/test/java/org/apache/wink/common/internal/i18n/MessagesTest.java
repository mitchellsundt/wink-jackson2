...
public class MessagesTest extends TestCase {
    ...
    private static String defaultResourceFile = "wink-jackson2-common/src/main/resources/org/apache/wink/common/internal/i18n/resource.properties";
    static {
    ...
    private static class MessageStringsCache {
        ...
        public String getFormattedStringByKey(String key, String filePath) {
            ...
            if (formattedString == null) {
                fail("Expected to find non-null property with key " + key + " used by " + filePath);
            } else if (formattedString.equals("")) {
                fail("Expected to find non-empty property with key " + key + " used by " + filePath);
            }
    ...
    private String getWorkspacePath() {
        if (workSpacePath == null) {
            ...
            int lenDrop = 2 + parts[parts.length-1].length() +
            		parts[parts.length-2].length() + parts[parts.length-3].length();
            workSpacePath = workSpacePath.substring(0, workSpacePath.length() - lenDrop);
        }
