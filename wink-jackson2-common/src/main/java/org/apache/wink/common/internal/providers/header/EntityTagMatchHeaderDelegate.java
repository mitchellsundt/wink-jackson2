...
public class EntityTagMatchHeaderDelegate implements HeaderDelegate<EntityTagMatchHeader> {
    ...
    private static HeaderDelegate<EntityTag> doNotDirectlyAccess = null;

    public static synchronized HeaderDelegate<EntityTag> getEntityTagHeaderDelegate() {
        if (doNotDirectlyAccess == null) {
            RuntimeDelegate delegate = RuntimeDelegate.getInstance();
            doNotDirectlyAccess = delegate.createHeaderDelegate(EntityTag.class);
        }
    ...
    public EntityTagMatchHeader fromString(String value) throws IllegalArgumentException {
        ...
        for (String token : valueTokens) {
            ifMatchHeader.addETag(getEntityTagHeaderDelegate().fromString(token));
        }
